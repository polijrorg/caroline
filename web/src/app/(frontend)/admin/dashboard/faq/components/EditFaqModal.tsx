...
import { Faq, UpdateFaqDTO } from "@/types/faq";

interface EditFaqModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (id: string, data: UpdateFaqDTO) => Promise<any>;
  faq: Faq | null;
}

export function EditFaqModal({ isOpen, onClose, onSubmit, faq }: EditFaqModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit(faq.id, formData);
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b">
          <h2 className="text-xl font-semibold">Editar FAQ</h2>
          <button
            onClick={handleClose}
            disabled={loading}
